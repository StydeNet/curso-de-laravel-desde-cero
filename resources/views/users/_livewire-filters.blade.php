        ...
        <div class="col-md-6">
            @foreach (trans('users.filters.states') as $value => $text)
                <div class="form-check form-check-inline">
                    <input wire:click="$emit('refreshUserList', 'state', $event.target.value)" class="form-check-input" type="radio" name="state" id="state_{{ $value }}" value="{{ $value }}" {{ $value == request('state') ? 'checked' : '' }}>
    ...
    <div class="row row-filters">
        <div class="col-md-6">
            <div class="form-inline form-search">
                <input wire:keyup.debounce.500ms="$emit('refreshUserList', 'search', $event.target.value)" type="text" name="search" value="{{ request('search') }}" class="form-control form-control-sm" placeholder="Buscar...">
                &nbsp;
                <div class="btn-group">
                    <select wire:change="$emit('refreshUserList', 'role', $event.target.value)" name="role" id="role" class="select-field">
                ...
                <div class="btn-group drop-skills">
                    ...
                    @foreach($skillsList as $skill)
                        <div class="form-group form-check">
                            <input wire:click="$emit('refreshUserList', 'skills', $event.target.value, $event.target.checked)" name="skills[]"
                                   type="checkbox"
